                    ...
                    <v-card class="mx-auto px-2 text-center" width="100%" max-width="600">
                            ...
                            <v-col>
                                    ...
                                    <v-col>
                                        ...
                                        <v-row>
                                            <v-col class="text-center mt-n7">
                                                <span class="text-h4">{{ parseToStringDate(new
                                                    Date(reservation.startTime)) }}</span>
                                            </v-col>
                                        </v-row>
                                    </v-col>
                                </v-row>
                                ...
                                <v-row class="d-flex align-center my-2">
                                    ...
                                    <v-col class="ml-n4 mr-7">
                                        <span class="text-h6">Hora de inicio: {{
                                            getHoursAndMinsFromDate(reservation.startTime) }}</span>
                                    </v-col>
                                    <v-col cols="1">
                                        <v-icon size="small" icon="mdi-weather-night" />
                                    </v-col>
                                    <v-col class="ml-n7 mr-7">
                                        <span class="text-h6">Hora de fin: {{
                                            getHoursAndMinsFromDate(reservation.endTime) }}</span>
                                    </v-col>
                                </v-row>
                                <v-divider />
                                <v-row class="mt-5 mb-n8">
                                    <v-col cols="5">
                                        <v-text-field v-model.number="reservationSeats" label="Número de asientos"
                                            prepend-icon="mdi-table-chair" type="number" variant="outlined"
                                            density="compact" required
                                            @input="reservationSeats = Math.max(1, reservationSeats)" />
                                    </v-col>
                                    <v-col v-if="space.admitsRepetition">
                                        <v-select v-model="repetition" :items="repetitionOptions" item-title="label"
                                            item-value="value" label="Repetición" prepend-icon="mdi-repeat"
                                ...
                                <v-row class="mt-n3 mb-n4">
                                    <v-col>
                                        <v-fade-transition>
                                            <v-alert v-if="reservationSeats >= maxSeatsAllowed" type="warning"
                                                density="compact" variant="tonal">
                                                No se pueden reservar más de {{ maxSeatsAllowed }} asientos.
                                            </v-alert>
                                            <v-alert v-if="reservedModal" type="warning" density="compact"
                                                variant="tonal">
                                                No se puede reservar periódicamente, ya hay una reserva con el mismo
                                                horario.
                                            </v-alert>
                                        </v-fade-transition>
                                    </v-col>
                        ...
                        <v-card-actions>
                            <v-row class="mt-n6 mb-3 mr-2 d-flex justify-end ga-3">
                                ...
                                <TonalButton color="blue" text="Reservar" :loading="isLoading"
...
onMounted(() => {
    ...
    if (!space.value || !reservation.value) {
        ...
        getReservationsByDate(reservation.value.startTime);
...
const getReservationsByDate = async (dateParam) => {
    try {
        const response = await reservationService.getReservationsByDate(dateParam);
        reservationsByDate.value = response.data.reservations;

        reservationsByDate.value = reservationsByDate.value.filter(reservation =>
            reservation.spaceId == space.value._id
        );
...
function checkPeriodicReservations() {
    if (periodicReservations.value.length > 0) {
        ...
        for (let periodicReservation of periodicReservations.value) {
            ...
            //   - Reserva semanal: tengo que comprobar el día de la semana y la hora.
            ...
            // Comienza la resvisión de los horarios
...
async function sendReservation(formData) {
    ...
    if (repetition.value === 'no_repeat')
        ...
        // Calculamos el campo lastOccurrenceGenerated. 
