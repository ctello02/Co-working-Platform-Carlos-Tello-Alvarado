    ...
    <v-container class="container">
        <v-card v-if="user" class="mx-auto px-2" max-width="500">
            ...
            <v-card-text>
                <v-col>
                    <v-row>
                        <v-text-field v-model="newUser.name" type="text" variant="outlined" label="Nombre"
                            :prepend-icon="newUser?.isCompany ? 'mdi-office-building-outline' : 'mdi-account-circle-outline'"
                            required :rules="nameRules" class="my-1" />
                    </v-row>
                    <v-row>
                        <v-text-field v-model="newUser.email" variant="outlined" label="E-mail"
                            prepend-icon="mdi-email-outline" required class="my-1" :rules="emailRules" />
                    </v-row>
                    <v-row>
                        <v-text-field v-model="newUser.phone" variant="outlined" label="Teléfono"
                            prepend-icon="mdi-phone-outline" required class="my-1" :rules="phoneRules" />
                    </v-row>
                    <v-row>
                        <v-text-field v-model="newUser.address" variant="outlined" label="Dirección"
                            :prepend-icon="newUser?.isCompany ? 'mdi-map-marker-outline' : 'mdi-home-outline'" required
                            class="my-1" :rules="addressRules" />
                    </v-row>
                    ...
                    <v-row>
                        <v-text-field v-if="newUser.isCompany" variant="outlined" v-model="newUser.cif"
                            prepend-icon="mdi-file-document-outline" label="CIF" required :rules="cifRules"
                            class="mt-n5 mb-2" />
                    </v-row>
                </v-col>
            </v-card-text>
